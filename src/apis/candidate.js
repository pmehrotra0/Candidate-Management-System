...
export async function fetchCandidateDetails(limit=10, skip=0, sorting, columnFilters){
    ...
    console.log(`https://dummyjson.com/users${filter === "" ? '?' : filter+'&'}limit=${limit}&skip=${skip}${sorting.length !== 0 ? '&sortBy='+sorting[0].id+'&order='+(sorting[0].desc ? 'desc' : 'asc') : ""}`)
...
export async function searchCandidateDetails(globalFilter){
    console.log(`https://dummyjson.com/users${globalFilter ? '/search?q='+globalFilter : ""}`)
